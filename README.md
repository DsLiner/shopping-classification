...
팀 송골매의 `쇼핑몰 상품 카테고리 분류` 대회 참가용 코드입니다. 전처리와 간단한 분류기 그리고 평가 코드로 구성되어 있습니다. (코드는 python2.7, keras, tensorflow, konlpy 기준으로 작성되었습니다.)
...
@ base train-ratio 1.0
1.train-------------------------------------------------
python data.py make_db train bm
python data.py make_db train s
python data.py make_db train d

python classifier.py train ../data/train/bm ../data/model/bm bm false
python classifier.py train ../data/train/s ../data/model/s s false
python classifier.py train ../data/train/d ../data/model/d d false

2.test ---------------------------------------------------------
python data.py make_db test --train_ratio=0.0 bm
python data.py make_db test --train_ratio=0.0 s
python data.py make_db test --train_ratio=0.0 d

python classifier.py predict ../data/train/bm ../data/model/bm ../data/test/bm test bmcate.tsv bm
...
python data.py make_db dev --train_ratio=0.0 s
python data.py make_db dev --train_ratio=0.0 d

python classifier.py predict ../data/train/bm ../data/model/bm ../data/dev/bm dev bmcate.tsv bm
python classifier.py predict ../data/train/s ../data/model/s ../data/dev/s dev scate.tsv s
python classifier.py predict ../data/train/d ../data/model/d ../data/dev/d dev dcate.tsv d
...
2. `python data.py make_db train`
...
3. `python classifier.py train ./data/train ./model/train`
    - `./data/train`에 생성한 데이터셋으로 학습을 진행합니다.
    - 완성된 모델은 `./model/train`에 위치합니다.
4. `python classifier.py predict ./data/train ./model/train ./data/train/ dev predict.tsv`
    - 단계 1. 에서 `train`의 20%로 생성한 평가 데이터에 대해서 예측한 결과를 `predict.tsv`에 저장합니다.
5. `python evaluate.py evaluate predict.tsv ./data/train/data.h5py dev ./data/y_vocab.cPickle`
